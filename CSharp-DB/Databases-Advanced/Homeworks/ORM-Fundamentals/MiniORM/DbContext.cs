...
namespace MiniORM
{
    ...
    public abstract class DbContext
    {
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            var foreingKey = entityType.GetProperties()
            ...
            {
                var primaryKeyValue = foreingKey.GetValue(entity);
            ...
            where TEntity : class, new()
        {
            ...
            var foreingKeys = entityType.GetProperties()
                .Where(pi => pi.HasAttribute<ForeignKeyAttribute>())
                .ToArray();

            foreach (var foreignKey in foreingKeys)
            {
                var navigationProperyName = foreignKey
                ...
                foreach (var entity in dbSet)
                {
                    var foreingKeyValue = foreignKey.GetValue(entity);

                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet)
                        .First(currentNavigationProperty => navigationPrimaryKey.GetValue(currentNavigationProperty).Equals(foreingKeyValue));
